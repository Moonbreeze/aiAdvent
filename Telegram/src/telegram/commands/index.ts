...
import { registerSwitchCommand } from './switch';
...
export const commands: BotCommand[] = [
	...
	{
		command: 'switch',
		description: 'Переключить провайдер LLM',
		register: registerSwitchCommand,
	},
	{
		command: 'close',
...
export const registerAllCommands = (bot: Bot<Context>): void => {
	...
	bot.api.setMyCommands(commands.map(({ command, description }) => ({ command, description })));
};
