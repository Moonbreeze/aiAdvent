...
import { sessionManager, isOutputMode } from '../../services';
import { OutputMode } from '../../core/session/types';
...
export const registerModeCommand = (bot: Bot<Context>) => {
	bot.command('mode', async (ctx) => {
		...
		if (!isOutputMode(requestedMode)) {
			await ctx.reply(`Неизвестный режим: ${requestedMode}\n\n` + `Доступные режимы: ${OutputMode.join(', ')}`);
			return;
		}
